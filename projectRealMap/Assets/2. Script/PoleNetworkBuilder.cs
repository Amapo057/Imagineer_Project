...
{
    [Header("런타임 자동 생성")]
    [Tooltip("Play 시작 시 자동으로 전봇대 네트워크를 다시 생성합니다.")]
    ...
    [Header("대각선 연결 설정")]
    [Tooltip("꺼져 있으면 상/하/좌/우에 가까운 연결만 허용합니다. 켜면 대각선 연결도 허용합니다.")]
    public bool allowDiagonalConnections = false;

    [Tooltip("대각선 연결을 막을 때 사용하는 축 판정 값입니다. 값이 높을수록 가로/세로에 가까운 연결만 허용합니다.")]
    [Range(0.5f, 1f)]
    ...
    private bool IsCandidateValid(PoleNode origin, PoleNode candidate)
    {
        Vector3 toCandidate = candidate.Position - origin.Position;
        toCandidate.y = 0f;
        ...
        if (!allowDiagonalConnections)
        {
            if (!IsAxisAlignedEnough(toCandidate.normalized))
            {
                if (showDebugLog)
                {
                    Debug.Log("대각선 후보 제외: " + origin.name + " - " + candidate.name);
                }
    ...
    private bool IsAxisAlignedEnough(Vector3 direction)
    {
        ...
        float strongestAxisAmount = Mathf.Max(xAmount, zAmount);

        return strongestAxisAmount >= axisStrictness;
    }
